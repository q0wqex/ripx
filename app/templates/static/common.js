...
// convertMp4ToGif открывает модальное окно для выбора MP4 файла
...
// closeConvertModal закрывает модальное окно и сбрасывает его состояние
...
// submitMp4Conversion отправляет MP4 файл на сервер и обрабатывает ответ
function submitMp4Conversion() {
  const fileInput = document.getElementById('mp4-file');
  const statusDiv = document.getElementById('convert-status');
  ...
  if (!fileInput.files || fileInput.files.length === 0) {
    statusDiv.innerHTML = '<p style="color: red;">Пожалуйста, выберите MP4 файл</p>';
    return;
  }
  ...
  if (file.size > maxSize) {
    statusDiv.innerHTML = '<p style="color: red;">Размер файла не должен превышать 50MB</p>';
    return;
  }
  ...
  statusDiv.innerHTML = '<p>Конвертация...</p>';
  ...
  .then(data => {
    if (data.success) {
      statusDiv.innerHTML = `
        ...
        <p><a href="${data.gifUrl}" target="_blank">Открыть GIF</a></p>
        <img src="${data.gifUrl}" alt="Конвертированный GIF" style="max-width: 100%;">
      `;
    } else {
